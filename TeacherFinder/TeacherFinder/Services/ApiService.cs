...
namespace TeacherFinder.Services
{
    public class ApiService
    {
        public async Task<bool> RegisterUser(string email, string password, string confirmPassword)
        {
            ...
            var response = await httpClient.PostAsync("https://teacherapi.azurewebsites.net/api/Account/Register", content);
        ...
        public async Task<TokenResponse> GetToken(string email, string password)
        {
            ...
            var response = await httpClient.PostAsync("https://teacherapi.azurewebsites.net/Token", content);
        ...
        {
            var httpClient = new HttpClient();
            var recoverPasswordModel = new PasswordRecoveryModel() { Email = email };
            var json = JsonConvert.SerializeObject(recoverPasswordModel);
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await httpClient.PostAsync("https://teacherapi.azurewebsites.net/api/Users/PasswordRecovery", content);
        ...
        public async Task<List<Instructor>> GetInstructors()
        {
            var httpClient = new HttpClient();
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", Preferences.Get("accesstoken", ""));
            var response = await httpClient.GetStringAsync("https://teacherapi.azurewebsites.net/api/instructors");
        ...
        {
            var httpClient = new HttpClient();
            var response = await httpClient.GetStringAsync("https://teacherapi.azurewebsites.net/api/cities");
        ...
        {
            var httpClient = new HttpClient();
            var response = await httpClient.GetStringAsync("https://teacherapi.azurewebsites.net/api/courses");
